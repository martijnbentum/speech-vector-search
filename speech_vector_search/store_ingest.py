...
def extract_echoframe_metadata(store, label, unit_type=None):
    ...
    for metadata in store.find_by_tag(label):
        rows.append({
            ...
            "echoframe_key": _metadata_echoframe_key(metadata),
...
def _resolve_echoframe_metadata(row):
    ...
    if hasattr(row, "phraser_key") and (
        hasattr(row, "format_echoframe_key")
        or hasattr(row, "echoframe_key")
        or hasattr(row, "entry_id")
    ):
        return row
...
def _metadata_echoframe_key(metadata):
    '''return one printable echoframe key string from metadata.'''
    if hasattr(metadata, "format_echoframe_key"):
        return metadata.format_echoframe_key()
    if hasattr(metadata, "echoframe_key"):
        value = metadata.echoframe_key
        ...
        if isinstance(value, str):
            return value
    if hasattr(metadata, "entry_id"):
        return metadata.entry_id
    raise ValueError("metadata must expose an echoframe key")

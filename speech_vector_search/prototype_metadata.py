...
def make_prototype_row(label, unit_type, source_phraser_keys,
    ...
    '''build one explicit prototype metadata row.
...
def validate_row(row):
    ...
    if row["n_occurrences"] != len(row["source_phraser_keys"]):
        m = "n_occurrences must match number of source_phraser_keys"
        raise ValueError(m)
    if len(row["source_phraser_keys"]) != len(row["source_echoframe_keys"]):
        m = "source_phraser_keys and source_echoframe_keys must match in length"
        raise ValueError(m)

...
def _path(directory, name, suffix):
    return io._make_path(directory, name, suffix, overwrite=True)
...
def test_save_and_load_prototypes_with_name():
    ...
    with tempfile.TemporaryDirectory() as directory:
        paths = io.save_prototypes(vectors, md, directory=directory,
            name="prototypes")
        loaded_vectors, loaded_metadata = io.load_prototypes(
            name="prototypes", directory=directory)
    assert paths["vectors"] == _path(directory, "prototypes", "vectors")
    assert paths["metadata"] == _path(directory, "prototypes", "metadata")
    assert np.allclose(loaded_vectors, vectors)
    assert [item.to_dict() for item in loaded_metadata] == [
        item.to_dict() for item in md]
...
def test_save_and_load_prototypes_with_default_name():
    ...
    with tempfile.TemporaryDirectory() as directory:
        paths = io.save_prototypes(vectors, md, directory=directory)
        loaded_vectors, loaded_metadata = io.load_prototypes(directory=directory)
    assert paths["vectors"] == _path(directory, "prototypes", "vectors")
    assert paths["metadata"] == _path(directory, "prototypes", "metadata")
    assert np.allclose(loaded_vectors, vectors)
    assert [item.to_dict() for item in loaded_metadata] == [
...
def test_save_prototypes_accepts_metadata_row_class():
    ...
    md = [metadata_module.PrototypeMetadata('a', 'word', ['e0'])]
    with tempfile.TemporaryDirectory() as directory:
        io.save_prototypes(vectors, md, directory=directory)
        _, loaded_metadata = io.load_prototypes(directory=directory)
    assert [item.to_dict() for item in loaded_metadata] == [md[0].to_dict()]
...
def test_save_prototypes_ignores_existing_config_when_config_is_none():
    ...
    with tempfile.TemporaryDirectory() as directory:
        config_path = _path(directory, "demo", "config")
        config_path.write_text("{}")
        paths = io.save_prototypes(vectors, md, directory=directory, name="demo")
...
def test_save_prototypes_creates_nested_directories():
    ...
    with tempfile.TemporaryDirectory() as directory:
        nested = Path(directory) / "runs" / "2026-04-07" / "demo"
        paths = io.save_prototypes(vectors, md, directory=nested, name="nested")
...
def test_prototype_paths_use_directory_layout():
    with tempfile.TemporaryDirectory() as directory:
        ...
        vectors_path = _path(directory, name, "vectors")
        metadata_path = _path(directory, name, "metadata")
        config_path = _path(directory, name, "config")
